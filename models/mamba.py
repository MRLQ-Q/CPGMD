...
def scan_permutation(xz_main, _perm):
    return xz_main[:, :, _perm].contiguous()

def merge_permutation(o_main, _perm_rev):
    return o_main[:, _perm_rev, :].contiguous()

class bow_CrossScan(torch.autograd.Function):
    @staticmethod
...
class bow_CrossMerge(torch.autograd.Function):
...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        xz_ = bow_CrossScan.apply(xz, self.token_list, self.origina_list)
        ...
        out = (out1 + out2) / 2
        out = bow_CrossMerge.apply(out, self.token_list, self.origina_list)
        return out
